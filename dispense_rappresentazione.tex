...
\begin{proof}
  Consideriamo $|e| = 1$. Dato che $H$ agisce transitivamente su $\mathbb{S}^2$, $\forall g \in SO(3) \exists h \in H$
  \[ ge = he \Rightarrow e = h^{-1}ge \Rightarrow h^{-1}g \in \Stab(e) \]

  Ma lo stabilizzatore di $e$ è una rotazione intorno a quell'asse, quindi è in $H$
\end{proof}
...
\textsc{Dimostrazione della proposizione \ref{prop:omo su2 so3}}
   \begin{enumerate}
   \item{
     Cerchiamo di usare il lemma che abbiamo appena mostrato. Dimostriamo che l'azione di $\rho(SU(2))$ su $\mathbb{S}^2$ è transitiva. Prendiamo una matrice $x$

     \[ x \in \mathbb{E} \cap \mathbb{S}^2\]

     Essendo una matrice autoaggiunta, per il teorema spettrale è unitariamente diagonalizzabile, ovvero $\exists g \in U(2) | gxg^{-1}$ è una matrice diagonale. In particolare, dato che $|\det(g)| = 1$, possiamo considerare $\hat g = \sqrt{\det(g)}g$ per dire che possiamo scegliere una matrice $\hat \rho \in SU(2)$. Nella sua forma diagonale la matrice sarà
     ...
     Mostriamo ora che $\rho(SU(2))$ contiene tutte le rotazioni intorno ad un asse. Questo è semplice in quanto sappiamo che $\mathbb{S}^1 \subset SU(2)$. Vediamo come agisce
   ...
   \item{
     Bisogna mostrare che il $\Ker$ dell'applicazione sono solo le matrici $\Id, -\Id$. Se vale $\rho(g) = \Id \quad \forall g \in SU(2)$, a maggior ragione vale 
     ...
     \right)
     \end{align*}
     E questa cosa è vera solo per $z_2 = 0$ e $|z_1| = 1$. Se andiamo a considerare il caso studiato poco fa, allora è evidente che deve essere solo $z_1 = \pm 1$
   ...
   \begin{rem}
     A questo punto possiamo affemare che $SU(2) / \Ker\rho \cong SO(3)$ come gruppi topologici e non solo come gruppi.
   \end{rem}
