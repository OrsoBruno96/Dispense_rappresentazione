...
\begin{enumerate}
	...
	\item Notiamo inoltre che $\chi_\rho$ è costante sulle classi di coniugio di $G$, ovvero: dati $s, t \in G$, se esiste un elemento $r \in G | rsr^{-1} = t$, allora si ha 

\[ \chi_\rho(s) = \chi_\rho(t) \]

La motivazione è semplice: sappiamo che $tr(M A M^{-1}) = tr(A)$ in quanto la traccia è la somma degli autovalori ed è quindi un invariante di similitudine.
...
\[\langle \rho|\sigma \rangle = \begin{cases}
...
\item Una rappresentazione di un gruppo $\rho$ è irriducibile $\Leftrightarrow \langle \chi_\rho | \chi_\rho \rangle = |\chi_\rho|^2 = 1$ 
