...
\begin{defn}[Algebra]
Un algebra è uno spazio vettoriale $A$ con un altro prodotto $\cdot : A \times A \to A$ oltre a quello per uno scalare che rispetti alcune proprietà. In particolare il prodotto deve essere associativo (non necessariamente commutativo), bilineare rispetto alle caratteristiche di spazio vettoriale.

Se esiste l'elemento neutro del prodotto, allora l'algebra si chiama unitaria.
\end{defn}
...
\begin{exmp}
Gli esempi più standard di algebra di divisione di dimensione finita sono per l'appunto i campi $\R$ e $\C$ come spazi vettoriali reali e il corpo $\HH$ come spazio vettoriale su $\R$. Questo esempio non è così scontato, come si vedrà fra poco nel teorema \ref{thm: frobenius}
\end{exmp}


\begin{rem}
Consideriamo una rappresentazione $\rho, V_\rho$ irriducibile su $\R$ e consideriamo 

\[ End_G(V_\rho)\]

Possiamo a questo punto applicare la versione debole del lemma di Schur (quella forte vale per rappresentazioni su $\C$) e dire che ogni elemento di $End_G(V_\rho)$ o è la funzione nulla oppure è un isomorfismo, quindi ammette inverso. Per questo motivo $End_G(V_\rho)$ è un'algebra di divisione. Se il gruppo è finito, ovviamente l'algebra sarà di dimensione finita. 
\end{rem}


A questo punto vale la pena di dimostrare il seguente teorema di classificazione delle algebre di divisione.

\begin{thm}[di Frobenius]
\label{thm: frobenius}

Sia $A$ un algebra di divisione di dimensione finita. Allora si ha 
...
QUI NON HO CAPITO COME MOSTRA CHE IN OGNI CASO $\R$ \'E UNA SOTTOALGEBRA DI $A$. QUALCUNO LO SCRIVA. 


A questo punto sappiamo che  $\R \subseteq A$. Se $dim A = 1$ abbiamo finito, altrimenti possiamo assumere $dim A > 1$. 

Vorremmo mostrare che in ogni caso esiste un sottospazio di $A$ isomorfo a $\C$. Per farlo andiamo a considerare il seguente fatto. Indichiamo con $A[\alpha]$ la sottoalgebra generata da un generico elemento $\alpha \in A$, ovvero consideriamo l'insieme

\[A[\alpha] = \left\{ \dsum_{n = 0} ^N a_n \alpha^n | \ a_i \in \R, \alpha \in A   \right\} \]

In particolare siamo interessati a considerare le sottoalgebre generate dagli $\alpha$ che non sono in $\R$, in quanto per gli $\alpha \in \R$ semplicemente otterremmo di nuovo un numero reale. Notiamo che in ogni caso lo spazio $A[\alpha]$ è comunque contenuto in $A$ e quindi ha dimensione finita, in particolare esisterà un certo $N$ per cui gli $\alpha^n$ sono linearmente dipendenti. Prendiamo quindi il minimo $N$ per cui questo avviene, ovvero consideriamo


\[ \dsum_{n=0}^N a_n \alpha^n = 0 \]

Questo si può esprimere dicendo che il polinomio 

\[ p(x) = \dsum_{n = 0}^N a_n x^n \] 

ha come radice $\alpha$, ovvero $p(\alpha) = 0$. Tuttavia $N$ è minimo, quindi questo polinomio è anche irriducibile. Ma su $\R$ i polinomi irriducibili non sono tanti. In particolare può avere grado 1 o 2. Vediamo rapidamente perché non può avere grado 1. Supponiamo per assurdo che quindi sia

\[p(x) = a_0 + a_1 x\]

Ma noi sappiamo che $p(\alpha) = 0$. Questo è assurdo perché vorrebbe dire che $\alpha \in \R$, cosa opposta a quella che avevamo assunto. Di conseguenza $p(x)$ ha grado esattamente 2. Senza perdita di generalità lo prendiamo monico, ovvero 

\[ p(x) = a + bx + x^2\]

A questo punto non è difficile mostrare che $A[\alpha]$ ha esattamente dimensione $2$, in quanto se $\alpha^2$ si scrive in termini di potenze inferiori lo faranno anche tutte le altre. Inoltre dato che $p(x)$ è irriducibile dovrà pure avere  $\Delta < 0$.

A questo punto con un conto lungo ma banale si può mostrare che ogni elemento in $A[\alpha]$ ammette inverso in $A[\alpha]$, che è quindi un campo di dimensione $2$ che include $\R$. Da questo segue che 

\[ A[\alpha] \cong \C \]

Se $dim A =2 $ abbiamo finito. Altrimenti $dim A > 2$.
