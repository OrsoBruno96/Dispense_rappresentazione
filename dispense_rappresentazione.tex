...
PRIMA FACEVA QUALCHE CONSIDERAZIONE SULLA DIMENSIONE???
	...
	\end{align*}
	Sia $\varphi\in \End_G(V^{\oplus n})$, consideriamo $\varphi_i:V\to V$ definita come $\varphi_{ij} = p_i\circ\varphi\circ q_j\in\End_G(V)$:
	\[
	...
	\begin{align*}
		...
		\psi(\varphi)&\to (c_{ij})
	\end{align*}
	Mostriamo ora che $\psi$ è un isomorfismo di algebre: $\psi$ è surgettiva, infatti, data $(c_{ij})\in M_n(\K)$, se consideriamo $\varphi = \sum c_{ij} q_i\circ p_j$ abbiamo che $\psi(\varphi) = (c_{ij})$; dimostriamo che è un omomorfismo, infatti siano $\varphi_1 = \sum c_{ij} q_i\circ p_j$ e $\varphi_2 = \sum d_{ij} q_i\circ p_j$, allora $\varphi_1\circ\varphi_2 = \sum c_{ij} d_{hk}\ q_i\circ p_j\circ q_h\circ p_k = \sum c_{ij}d_{jk}\ q_i\circ p_k$ dove l'ultima uguaglianza si ottiene dal fatto che se $j=h$ allora $p_j\circ q_h=\Id_V$, altrimenti è l'endomorfismo nullo, infine $\phi(\varphi_1\circ \varphi_2) = \sum c_{ij}d_{jk}\  q_i\circ p_k$ come per il prodotto tra matrici; la verifica dell'iniettività è immediata.
\end{proof}
