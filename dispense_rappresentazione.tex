...
I cicli, più che essere dcome mettere i segni sopra le frecce in tikzpictureselle liste, vanno pensati come elementi di $S_n$, ovvero come funzioni bigettive da $\{1,\dots,n\}$ in sé.
...
Non è difficile verificare che se $\tau\in S_n$ è una trasposizione allora $f(\rho)$ e $f(\tau\rho)$ hanno parità divercome mettere i segni sopra le frecce in tikzpicturessa.
...
\end{rem}come mettere i segni sopra le frecce in tikzpictures
...
\textbf{Dimostrazione:} considero i seguenti due diagrammi:
 \tridiag{V\times W}{ \otimes }{V \otimes W}{  }{ V \overline{\otimes} W }{\overline{\otimes}} \tridiag{V\times W}{ \overline{\otimes} }{V \overline{\otimes} W}{}{V \otimes W}{\otimes}

%
%
%non so come mettere i due diagrammi nella stessa riga
%
%
...
\begin{tikzpicture}
\matrix(m)[matrix of math nodes,row sep=2em,column sep=3em,minimum width=3em]
{V\times W & V\otimes W\\
           & V\overline{\otimes} W \\
           & V\otimes W  \\ 
};
\path[-stealth]
(m-1-1) edge node [left] {$\otimes$} (m-1-2)
(m-1-1) edge node [right] { $\overline{\otimes}$ } (m-2-2)
(m-1-1) edge node [left] { $\otimes$ } (m-3-2)
(m-1-2) edge node [right] {$\phi$} (m-2-2)
(m-2-2) edge node [right] {$\psi$} (m-3-2);
\end{tikzpicture}     \ \    \begin{tikzpicture}
\matrix(m)[matrix of math nodes,row sep=2em,column sep=3em,minimum width=3em]
{V\times W & V\overline{\otimes} W\\
           & V\otimes W \\
           & V\overline{\otimes} W  \\ 
};
\path[-stealth]
(m-1-1) edge node [left] {$\overline{\otimes}$} (m-1-2)
(m-1-1) edge node [right] { $\otimes$ } (m-2-2)
(m-1-1) edge node [left] { $\overline{\otimes}$ } (m-3-2)
(m-1-2) edge node [right] {$\psi$} (m-2-2)
(m-2-2) edge node [right] {$\phi$} (m-3-2);
\end{tikzpicture}
...
\[<\{v\otimes w:v\in V,w\in W\}>=<A>\]
voglio quindi dimostrare che $Z=V\otimes W$. Considero il diagramma:
\tridiag{V\times W}{ \otimes }{V \otimes W}{  }{ <A> }{h}
dove $h(v,w)=v\otimes w$ e dato che $A$ genera $<A>$ abbiamo ben definito una applicazione bilineare. Quindi $\exists !\phi:V\otimes W\rightarrow <A>$ che fa commutare il diagramma. Detta $i:<A>\rightarrow V\otimes W$ l'inclusione consideriamo il seguente diagramma:\\
\begin{tikzpicture}
\matrix(m)[matrix of math nodes,row sep=2em,column sep=3em,minimum width=3em]
{V\times W & V\otimes W\\
           & <A>\\
...
\path[-stealth]
(m-1-1) edge node [left] {$\otimes$} (m-1-2)
(m-1-1) edge node [right] { $h$} (m-2-2)
(m-1-1) edge node [left] { $i\circ h$ } (m-3-2)
(m-1-2) edge node [right] {$\psi$} (m-2-2)
(m-2-2) edge node [right] {$\ i$} (m-3-2);
\end{tikzpicture}\\ tuttavia il diagramma grande commuta anche con $id_{V\otimes W}$ e quindi $i\circ\psi=id_{V\otimes W}\Rightarrow i$ è surgettiva $\Rightarrow <A>=V\otimes W$.
...
\[
	\rho_{\HH}(i) = \begin{pmatrix}
	          	i & 0\\
	          	0 & -i\\come mettere i segni sopra le frecce in tikzpictures
	          \end{pmatrix}
	\qquad
...
\begin{defn}
Dato $g\in G$ definiamo $V_g=\{f\in V : Supp(f)\subseteq gH \}$.come mettere i segni sopra le frecce in tikzpictures
\end{defn}
...
Dove tutto funziona uguale se al posto di $\R$ mettiamo $\C$ e per $\C_\R$ si intendono le funzioni continue. La mappa $I$ si dice integrazione se rispetta le seguenti proprietà.
