...
\textbf{Esempio:} $V=W=\C$ e come $\otimes$ prendo il prodotto ovvero $v\otimes w=vw$.
...
\end{prop}
\textbf{Dimostrazione:} considero i seguenti due diagrammi:
...
La proprietà universale per il prodotto tensore $\otimes$ dice che $\exists !\phi:V\otimes W\rightarrow V\overline{\otimes}W$ lineare che fa commutare il primo diagramma: analogamente $\exists !\psi:V\overline{\otimes}\rightarrow V\otimes W$ lineare che fa commutare il secondo diagramma. Ora consideriamo i seguenti due diagrammi:
...
Tuttavia ci rendiamo conto che sempre per la proprietà universale già l'identità fa commutare i due diagrammi "più grandi" e che quindi per unicità possiamo concludere che $\phi\psi=\psi\phi=id\Rightarrow \phi=\psi^{-1}$ per unicità e $\phi$ è un isomorfismo. \qed\\ 
Quindi quello che abbiamo dimostrato si traduce in
\begin{thm}
...
\textbf{Dimostrazione:} Definisco
\[\langle\{v\otimes w:v\in V,w\in W\}\rangle=\langle A\rangle\]
voglio quindi dimostrare che $Z=V\otimes W$. Considero il diagramma:
\[\tridiag{V\times W}{ \otimes }{V \otimes W}{  }{ \langle A\rangle }{h}\]
dove $h(v,w)=v\otimes w$ e dato che $A$ genera $\langle A\rangle$ abbiamo ben definito una applicazione bilineare. Quindi $\exists !\phi:V\otimes W\rightarrow \langle A\rangle$ che fa commutare il diagramma. Detta $i:\langle A\rangle\rightarrow V\otimes W$ l'inclusione consideriamo il seguente diagramma:\\
...
\[ 
\begin{cases}
O_2(\R) \cong \mathbb{S}^1 \\
U_2(\C) \cong \mathbb{S}^3 \\
\end{cases}
\]

A questo punto sarà opportuno definire una generalizzazione delle rappresentazioni usando lo strumento topologico che abbiamo a disposizione. 
...
\item Un'altra rappresentazione banale: la rappresentazione del gruppo $GL(V)$ in $GL(V)$ tramite l'identità, che è evidentemente continua
...
\begin{prop} Le rappresentazioni irriducibili di $\mathbb{S}^1$ sono tutte e sole le $\rho_n$ tali che

\[ \rho_n(z) = z^n \qquad \forall n \in \Z\]

\end{prop}


\textsc{Dimostrazione:}
Le prime osservazioni ovvie da fare sono le seguenti: le rappresentazioni irriducibili avranno dimensione $1$, per cui saranno omomorfismi in $\C^*$. Inoltre dovrà per forza essere $|\rho(z)| = 1 \quad \forall z \in \mathbb{S}^1$. Questo si dimostra abbastanza facilmente. Supponiamo per assurdo infatti che si abbia
...
Di conseguenza sappiamo che sicuramente $\rho(\mathbb{S}^1) \subseteq \mathbb{S}^1$
A questo punto per andare avanti con la dimostrazione dobbiamo utilizzare delle nozioni di topologia che non abbiamo ancora. In particolare dobbiamo utilizzare la nozione di rivestimento universale. Consideriamo la mappa
...
Vorremo ora quindi trovare $\theta: \R \to \R$ che sia un omomorfismo di rappresentazioni continue, per procedere con la dimostrazione. Graficamente possiamo vedere la situazione in questo modo
...
Il nostro obiettivo per procedere è quindi di trovare quali sono le funzioni $\theta$ che fanno commutare il diagramma. Questa $\theta$, se esiste, dovrà rispettare la seguente proprietà
...
In particolare, $\theta$ è continua e l'espressione sopra ha immagine puntiforme. Per questo motivo l'espressione precedente deve essere costante. 

\[ \theta(x + y) = \theta(x ) + \theta (y) + c \qquad c \in 2\pi \Z, \forall x,y\in \R\]

Usando alcuni trucchetti possiamo mostrare che deve essere $c = 0$. \footnote{Infatti possiamo scrivere $\theta(x + 0) = \theta(0) + \theta(x) + c \Rightarrow \theta(0) = -c$. Una volta fatto questo scriviamo $\theta(x-x) = \theta(x) - \theta(x) +c \Rightarrow c = -c$.}Una volta fatto questo abbiamo quasi finito, infatti

\[ \theta(x+y) = \theta(x) + \theta(y)\]

ha come soluzioni continue solo $\theta(x) = \alpha x$, con $\alpha \in \R$. Non solo, possiamo ancora dare delle informazioni a riguardo di $\alpha$. Infatti, dato che $\mathbb{S}^1$ è "periodico", deve essere

\[ 1 = \hat \rho(1) = \hat \rho(2\pi) = e^{i\alpha 2\pi}\]

Per cui in realtà si ha $\alpha \in \Z$. Per questo motivo per ogni $n \in \Z$ possiamo definire

\[ \hat \rho_n (x) = e^{i2\pi nx}\]

Per cui per ogni $n$ otteniamo una $\rho_n$ che fa commutare il diagramma se $\rho_n(z) = z^n$
...
Sia $G$ un gruppo topologico. Consideriamo un'applicazione $I$ 

\[I: C_{\R}(G) \to \R \]

\footnote{Tutto funziona allo stesso modo se al posto di $\R$ mettiamo $\C$. In futuro non si baderà a questa distinzione.} dove con $C_\R$ si intendono le funzioni continue da $G$ in $\R$. La mappa $I$ si dice integrazione se rispetta le seguenti proprietà.
...
Se questa integrazione esiste, allora in analogia a quanto fatto per i gruppi finiti possiamo enunciare e dimostrare il seguente teorema. 
...
\textsc{Dimostrazione:}
La dimostrazione è identica alla dimostrazione \ref{thm:gruppo finito completamente riducibile}, in cui al posto di $h_G$ si utilizza $I$. \qed
