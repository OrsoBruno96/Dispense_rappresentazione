...
E questo isomorfismo segue semplicemente dalle proprietà del prodotto tensore di spazi vettoriali. Definiamo su $Z$\footnote{Dato che sono spazi vettoriali in questo caso si tratta semplicemente di applicazioni lineari}$ = Hom(V_\rho, V_\sigma)$ la seguente rappresentazione: se $f \in Z$, allora possiamo definire la rappresentazione $\tau, V_\tau = Z$ di $G$ in questo modo

\[ \tau(s)f = \sigma(s) \circ f \circ \rho^{-1}(s)\]

Si verifica manualmente che sia una buona definizione. Ora volgliamo vedere che $Z$ e $V_{\rho^*\sigma}$ sono isomorfi anche come rappresentazioni. Chiamo $\Psi:V_\rho^* \otimes V_\sigma\rightarrow Hom(V_\rho, V_\sigma)$ la mappa tale che

\[ \Psi(\phi \otimes v ) (w) = \phi(w) v \qquad \forall \phi \in V_\rho^*, \forall v \in V_\sigma, \forall w \in V_\rho\]

Voglio mostrare che $\Psi$ è un isomorfismo di rappresentazioni. Innanzitutto notiamo che è stato sufficiente definirlo per i tensori decomponibili, dato che per il resto dello spazio basta estenderlo per linearità.
Precedentemente abbiamo già mostrato che $\Psi$ è un isomorfismo di spazi vettoriali. Quindi per concludere la dimostrazine è sufficiente dimostrare che è un omomorfismo di rappresentazione ovvero



\[ \Psi((\rho^*\otimes\sigma(s))(\phi\otimes v))=\tau(s)(\Psi(\phi\otimes v)) \qquad \forall v\in V_{\sigma}, \phi\in V^*_{\rho}, s\in G \]
e questo è vero sse
\[ \Psi((\rho^*\otimes\sigma(s))(\phi\otimes v)) (w)=\tau(s)(\Psi(\phi\otimes v)) (w) \qquad \forall w\in V_{\rho}, v\in V_{\sigma}, \phi\in V^*_{\rho}, s\in G \]
Riscriviamo il secondo membro
\[ \tau(s)(\Psi(\phi\otimes v)) (w)= \sigma(s)\circ  \Psi(\phi\otimes v) \circ \rho^{-1}(s) (w) = \sigma(s)( \phi(\rho^{-1}(s)(w))v )=\phi(\rho^{-1}(s)(w)) \sigma(s)(v) \]
ed ora il primo
\[ \Psi(\rho^*\otimes\sigma(s)(\phi\otimes v)) (w)= \Psi (\rho^*(s) (\phi) \otimes \sigma(s)(v)) (w) = \rho^*(s)(\phi)(w)\sigma(s)(v) \]
e dato che l'applicazione duale è stata definita come l'unica tale che 
\[ \rho^*_s(\phi)\circ \rho_s=\phi\]
allora i due membri sono uguali e $\Psi$ è un omomorfismo di rappresentazione. Ne segue che 
...
\[ tr(T)=tr (\dfrac{1}{|G|} \dsum_{s\in G} \rho(s))= \dfrac{1}{|G|} \dsum_{s\in G} tr(\rho(s))= \langle \chi_\rho| 1 \rangle \]
...
\begin{lemma}
Sia $(\rho, V_\rho)$ una rappresentazione irriducibile di $G$ su $\C$. Allora ogni forma bilineare non nulla $G-$invariante è non degenere. Inoltre è unica a meno di scalari, ovvero se ne esiste almeno una, si ha $dim (V^*\otimes V^*)^G=1$ .
\end{lemma}
