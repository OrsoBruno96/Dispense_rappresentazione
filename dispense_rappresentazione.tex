...
A questo punto possiamo andare a cercare i sottospazi $G-$invarianti di $\Hom(V_\rho, V_\sigma)$, che non sono altro che le sue sottorappresentazioni irriducibili. In particolare stiamo quindi cercando dei sottospazi $W \subset Z = Hom(V_\rho, V_\sigma)$ tali che $s\cdot W \subset W \quad \forall s \in G$ 

In particolare, cerchiamo le funzioni $f \in Hom(V_\sigma, V_\rho)$ tali che $s\cdot f = f$. Ricordando come è definita l'azione di $G$ su $\Hom(V_\rho, V_\sigma)$ si vede che

\[f = s\cdot f =  \rho(s) \circ f \circ \sigma^{-1}(s) \Rightarrow  f \sigma(s)= \rho(s) f\] 


Ovvero le applicazioni $f$ invarianti per l'azione di $G$ sono gli omomorfismi di rappresentazioni da $(\rho, V_\rho) $ a $(\sigma, V_\sigma)$
...
\item $V$ possiede una forma bilineare simmetrica $G-$invariante. Ovvero lo spazio delle forme bilineari simmetriche $S^2V^*\subset V\otimes V$ è tale che $(S^2V^*)^G\neq 0$.
\end{enumerate} 
\end{thm}
...
\begin{lemma}
Sia $V$ una $G-$rappresentazione $/\C$. Allora ogni forma bilineare non nulla $G-$invariante è non degenere. Inoltre, se esiste, è unica a meno di scalari, ovvero $\dim (V^*\otimes V^*)^G=1$.
\end{lemma}
\begin{proof}
...
Sia $V$ una rappresentazione $/\C$ irriducibile: quando è che esiste una forma bilineare $G-$invariante? Ovvero quand'è che si ha $\left(V^* \otimes V^*\right)^G \neq 0$? Cerchiamo delle condizioni necessarie...
\begin{lemma} 
...
e questo è vero $\Leftrightarrow $ $\chi_\rho$ è una funzione reale. E ciò è vero se la rappresentazione è definibile $/\R$.
\end{proof}
