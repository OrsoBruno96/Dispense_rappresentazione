...
\begin{enumerate}
\item $V\otimes U \cong U\otimes V$, in modo tale che i vettori $v\otimes u$ vengano mandati in $u\otimes v$;
\item $(V\otimes U)\otimes W \cong V\otimes(U\otimes W)$ con $(v\otimes u)\otimes w$ che viene mandato in $v\otimes(u\otimes w)$;
\item $(V\oplus U)\otimes W \cong (V\otimes W) \oplus (U\otimes W)$ con $(v,u)\otimes w$ che viene mandato in $(v\otimes w, u\otimes w)$.
\end{enumerate}
\end{exercise}
...
\begin{exercise}
Sia $V$ uno spazio vettoriale. Allora $V^*\otimes V^*$ è isomorfo allo spazio vettoriale delle forme bilineari $V\times V\to \K$.
\end{exercise}
...
\begin{enumerate}
	...
	\item imporre infine che $\rho:G\to GL(\C)$ sia veramente un omomorfismo di gruppi (per ora abbiamo solo posto delle condizioni necessarie):
	 per fare ciò bisogna controllare che le \emph{relazioni} che intercorrono tra i generatori siano rispettate nell'immagine.\footnote{questo in parole povere significa che tutte le regole con cui vengono moltiplicati gli elementi devono essere rispettate, per dare una spiegazione formale servirebbero i prodotti liberi che però non sono necessari per questo corso.}
	 In effetti già le condizioni imposte al punto 3 possono essere considerate come relazioni imposte dai generatori.
\end{enumerate}
Questo definisce un omomorfismo da $G$ a $\C^*$: dato $h\in G$ tale che $h=g_{i_1}^{a_1}\cdots g_{i_t}^{a_t}$, allora $\rho(h) = \rho(g_{i_1})^{a_1}\cdots \rho(g_{i_t})^{a_t}$.
\begin{rem}
...
\begin{defn}[Somma di rappresentazioni]
  ...
  \[ (\rho + \sigma)_g (v,w) = \rho_g(v) + \sigma_g(w) \qquad \forall (v,w) \in V\oplus W \]
...
Sia $\rho$ una rappresentazione di $G$ su $V_\rho$. Allora la rappresentazione duale $\rho^*$ è la rappresentazione di $G$ su $V_\rho ^*$ definita da:
...
\begin{defn}[Rappresentazione regolare]
  Consideriamo un gruppo finito $G$ e uno spazio vettoriale $V_\rho$ di dimensione $|G|$ su $\C$.
